...
from torch.distributed.fsdp import \
    FullyShardedDataParallel as \
    FSDP  # ShardedStateDictConfig, # un-flattened param but shards, usable by other parallel schemes.
...
def load_model_sharded(model, rank, cfg):
    ...
    folder_name = (cfg.dist_checkpoint_root_folder + "/" +
                   cfg.dist_checkpoint_folder + "-" + cfg.model_name)
    ...
    if not load_dir.exists():
        if rank == 0:
            print(
                f"No sharded_state_dict checkpoint directory found...skipping")
        return
    if rank == 0:
...
def save_fsdp_model_checkpoint_full(
    ...
    if rank == 0:
        ...
        save_name = cfg.model_name.replace("/",
                                           "--") + "-" + str(epoch) + ".pt"
        ...
        print(
            f"model checkpoint saved for epoch {epoch} at {save_full_path}\n")
...
def load_model_checkpoint(model, rank, cfg):
    ...
    full_state_dict_model_path = (Path.cwd() / cfg.checkpoint_folder /
                                  cfg.checkpoint_model_filename)
    # is it present...
...
def save_optimizer_checkpoint(model, optimizer, rank, cfg, epoch=1):
    ...
    if rank == 0:
        ...
        opt_save_name = ("optimizer" + "-" + cfg.model_name + "-" +
                         str(epoch) + ".pt")
